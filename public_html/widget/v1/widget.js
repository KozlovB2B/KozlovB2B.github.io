!(function () {
            ...
            function o(n, r, i) {
                ...
                if ("undefined" != typeof document) {
                    ...
                    for (var p = document.cookie ? document.cookie.split("; ") : [], u = /(%[0-9A-Z]{2})+/g, d = 0; d < p.length; d++) {
                        var f = p[d].split("="), l = f.slice(1).join("=");
    ...
    /**
     * Виджет, внедряющийся в страницу для работы со ScriptDesinger
     *
     * @constructor
     */
    var SSWidget = function () {

        var p = this;

        p.state_cookie_name = 'SSWidget_state';
        p.state = {};
        p.template = '<div id="{handlerId}"><div id="{titleId}">Script Designer</div><div id="{functionsWrapperId}"><ul><li class="{functionClass}" id="{functionHome}"></li><li class="{functionClass}" id="{functionMinimize}"></li><li class="{functionClass}" id="{functionMaximize}"></li></ul></div></div><iframe id="{iframeId}" src="{homeUrl}" marginheight="0" frameborder="0"></iframe>';


        //var host = 'ss';
        var host = 'www.scriptdesigner.ru';

        if (typeof window['SSWidget_dev_host'] !== 'undefined') {
            host = window['SSWidget_dev_host'];
        }

        p.config = {
            panelId: "sswidget-panel-container",
            panelClassName: "sswidget-panel-container",
            panelMinimizedClassName: "sswidget-panel-minimized",
            handlerId: "sswidget-panel-handler",
            titleId: "sswidget-panel-title",
            functionsWrapperId: "sswidget-panel-functions-wrapper",
            functionClass: "sswidget-panel-function",
            functionHome: "sswidget-panel-function-home",
            functionMinimize: "sswidget-panel-function-minimize",
            functionMaximize: "sswidget-panel-function-maximize",
            iframeId: "sswidget-panel-iframe",
            homeUrl: "//" + host + "/api/v1/script/index",
            assetsUrl: "//" + host + "/widget/v1/assets"
        };
    ...
    /**
     * Объект состояния по-умолчанию
     */
    SSWidget.prototype.getDefaultState = function () {
        var p = this;
    ...
    SSWidget.prototype.loadState = function () {
        ...
        var from_cookie = Cookies.get(p.state_cookie_name);

        if (from_cookie) {
            p.state = JSON.parse(from_cookie);
        } else {
            p.state = p.getDefaultState();
        }
    };

    /**
     * Запись состояния в кукис
     */
    SSWidget.prototype.writeState = function () {
        var p = this;

        Cookies.set(p.state_cookie_name, JSON.stringify(p.state), { expires: 7 });
    };
    ...
    SSWidget.prototype.render = function () {
        ...
        link.href = p.config.assetsUrl + "/css/style.css";
        link.type = "text/css";
        link.rel = "stylesheet";
        link.media = "screen";
        ...
        panel.id = p.config.panelId;
        panel.className = p.config.panelClassName;
        panel.innerHTML = p.template.fmt({
            handlerId: p.config.handlerId,
            iframeId: p.config.iframeId,
            iframeWrapperId: p.config.iframeWrapperId,
            homeUrl: p.state.location,
            titleId: p.config.titleId,
            functionsWrapperId: p.config.functionsWrapperId,
            functionClass: p.config.functionClass,
            functionHome: p.config.functionHome,
            functionMinimize: p.config.functionMinimize,
            functionMaximize: p.config.functionMaximize
        });

        if(p.state.translatey < 0){
            p.state.translatey = 0;
        }

        panel.style.webkitTransform = panel.style.transform = 'translate(' + p.state.translatex + 'px, ' + p.state.translatey + 'px)';
    ...
    SSWidget.prototype.minimize = function () {
        var p = this;
        var panel = document.getElementById(p.config.panelId);

        var w = panel.offsetWidth;
        var h = panel.offsetHeight;
        //
        //panel.setAttribute('data-maximazed-width', w);
        //panel.setAttribute('data-maximazed-height', h);
        //p.state.width = w;
        //p.state.height = h;

        panel.style.width = '180px';
        panel.style.height = '32px';
        panel.classList.add(p.config.panelMinimizedClassName);
     ...
     */
    SSWidget.prototype.maximize = function () {
        var p = this;

        var panel = document.getElementById(p.config.panelId);

        //var w_from_data = panel.getAttribute('data-maximazed-width');
        //var h_from_data = panel.getAttribute('data-maximazed-width');
        panel.style.width = p.state.width + 'px';
        panel.style.height = p.state.height + 'px';
        panel.classList.remove(p.config.panelMinimizedClassName);
        p.state.minimized = false;
    };


    SSWidget.prototype.sendDataToIframe = function () {
        var p = this;

        document.getElementById(p.config.iframeId).contentWindow.postMessage(JSON.stringify({
    ...
    SSWidget.prototype.setEvents = function () {
        ...
        document.getElementById(p.config.functionHome).onclick = function () {
            document.getElementById(p.config.iframeId).src = p.config.homeUrl;
        };

        document.getElementById(p.config.functionMinimize).onclick = function () {
            p.minimize();
        };

        document.getElementById(p.config.functionMaximize).onclick = function () {
            p.maximize();
        };


        // Посылаем данные в iframe если меняется адрес родительской страницы или происходит перезагрузка фрейма
        ...
        document.getElementById(p.config.iframeId).onload = function () {
            p.sendDataToIframe();
        };


        // Принимаем сообщения из фрейма и обновляем location в состоянии виджета
        ...
        eventer(messageEvent, function (e) {
            ...
            if (typeof data.location !== 'undefined') {
                p.state.location = data.location;
            }
        }, false);


        interact('#' + p.config.panelId)
            .draggable({
                onmove: function (event) {
                    var target = event.target,
                        x = (parseFloat(p.state.translatex) || 0) + event.dx,
                        y = (parseFloat(p.state.translatey) || 0) + event.dy;
                    ...
                    p.state.translatex = x;
                    p.state.translatey = y;
                },
                restrict: {
                    restriction: "parent",
            ...
            .on('resizemove', function (event) {
                ...
                p.state.width = event.rect.width;
                p.state.height = event.rect.height;
                //panel.setAttribute('data-maximazed-width', target.style.width);
                //panel.setAttribute('data-maximazed-height', target.style.height);
            });
    ...
    if (typeof window['ssWidgetInstance'] === 'undefined') {
        window.ssWidgetInstance = new SSWidget();
    }
})();